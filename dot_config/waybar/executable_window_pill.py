...
def print_status():
    ...
    text = (
        f"<span size='small' alpha='65%' rise='-1000'>{top_line}</span> "
        f"<span size='8500' weight='bold'>{bottom_line}</span>"
    )
...
# Initial Run
os.system("grep '@define-color' ~/.config/omarchy/current/theme/gtk.css > ~/.config/waybar/theme_colors.css")
print_status()
